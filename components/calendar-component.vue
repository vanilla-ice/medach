...
<script>
  ...
  var $headerInput = $('.header__search')
  var $headerBuffer = $('.header__search-buffer')
...
var newCalendar = function (year, attrCalendarsBody, calendarAllLinks, attrCalendarMonth, parentsYearClassName) {
    ...
    var dayLinks = $(calendarAllLinks)
    var classElementDayLinkDigit = parentsYearClassName

    for (var m = 0; dayLinks.length > m; m++) {
      var monthDigit = $(dayLinks[i]).parents('.calendar-digits').attr('data-month')
      var yearDigit = $(dayLinks[i]).parents(classElementDayLinkDigit).attr('data-year')

      $(dayLinks[m]).attr('href', 'index' + '#' + $(dayLinks[m]).text() + '/' + monthDigit + '/' + yearDigit)
    }
}
